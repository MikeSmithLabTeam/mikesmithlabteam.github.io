...
const colabs = Array.from(document.querySelectorAll("h2")).find(
  (el) => el.textContent === "Collaborators"
);
...
const addCollaborator = function(member, index){
  colabs.insertAdjacentHTML("afterend",
      `
      <blockquote>
      <div>
        <div class="summary-title">
        ...
        <div class="summary-container">
          ...
          <div class="summary-text">
          <br>
            <a href="${member.webpage}">${member.name}'s Webpage</a>
          </div>
        </div>   
      </div>
      </blockquote>
...
xhttp4.onreadystatechange = function () {
  if (this.readyState == 4 && this.status == 200) {
    ...
    response.colabs.reverse().forEach((member, index) => {
      addCollaborator(member, index + 1);
    });
